...
/* ------------------------------------------------------------- seo/ld+json -- */
// Shared structured-data builders, so every page's breadcrumbs are generated
// from the same trail it renders visually (or, for a page with no visual
// crumbs, from the one true fact of where it sits in the nav) rather than
// hand-typed twice and left free to drift apart.

export const ORG_ID = `${ORIGIN}/#organization`;

/**
 * BreadcrumbList JSON-LD. `trail` is the path after Home, as [name, url]
 * pairs — e.g. [['Capabilities', '/capabilities'], ['Web platforms',
 * '/capabilities/web-platforms']]. Home is prepended automatically.
 */
...
/** Service JSON-LD for one capability, referencing the org as provider. */
export function serviceSchema({ name, description, url, from, group }) {
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'Service',
    serviceType: name,
  ...
  const amount = String(from || '').match(/[\d,]+/);
  if (amount) {
    schema.offers = {
      '@type': 'Offer',
      priceCurrency: 'KES',
      price: amount[0].replace(/,/g, ''),
      priceSpecification: {
        '@type': 'UnitPriceSpecification',
        minPrice: amount[0].replace(/,/g, ''),
        priceCurrency: 'KES',
      },
    };
  }
 ...
 *   jsonLd       one structured-data object, or an array of them (each gets
 *                its own <script> tag) — e.g. [breadcrumbs(...), serviceSchema(...)]
 */
