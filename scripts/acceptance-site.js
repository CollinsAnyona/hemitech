...
async function main() {
  ...
  /* ---- structured data holds no placeholders -------------------------- */

  heading('Structured data');
  const ld = (index.match(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/) || [])[1];
  assert(!!ld, 'the homepage carries JSON-LD');
  if (ld) {
    const parsed = JSON.parse(ld);
    assert(!JSON.stringify(parsed).includes('['.concat('')) || !/\[[A-Z]/.test(JSON.stringify(parsed)),
      'no bracketed placeholder was emitted into structured data');
    assert(!('telephone' in parsed) && !('address' in parsed),
      'telephone and address are omitted rather than faked');
  }
